...
public class SailingBoatOverlay extends BoatOverlay {
    ...
    public SailingBoatOverlay(final MapWidget map, int zIndex, final BoatDTO boatDTO, Color color, CoordinateSystem coordinateSystem) {
        super(map, zIndex, boatDTO, color, coordinateSystem);
    ...
    protected void draw() {
        if (getMapProjection() != null && boatFix != null) {
            ...
            if (needToDraw(boatFix.legType, boatFix.tack, isSelected(), canvasWidth, canvasHeight, boatSizeScaleFactor,
                    color, displayMode)) {
                boatVectorGraphics.drawBoatToCanvas(getCanvas().getContext2d(), lastLegType, lastTack, getDisplayMode(),
                        (double) worldWidth, (double) worldWidth, boatSizeScaleFactor, color);
