 ...
 * self-hosted MapLibre tile server so the tile server's NGINX {@code secure_link} check can authenticate them. All the
 * tile-token domain logic lives here (secret parsing/validation, bucketing, MD5 signing, caching), keeping the
 * {@link Activator} responsible only for reading the configuration properties and handing them to this service.
 * <p>
 * The token is deliberately identical for every caller within one time bucket, so the same {@link MapTileAccessTokenDTO}
 * instance is returned across a bucket; because that DTO is a {@code CacheableRPCResult}, the result-caching RPC servlet
 * then serializes it only once per bucket. A token is signed to expire one further bucket length past the bucket
 * boundary, giving every token between one and two bucket lengths of remaining validity, so a client refreshing at half
 * the TTL always holds a comfortably valid token.
 * <p>
 * When no secret or no current key id is configured, this minter is in the disabled state: it always returns the
 * {@link #disabledToken auth-disabled token}, whose {@link MapTileAccessTokenDTO#isAuthenticationEnabled()} is
 * {@code false}, so the client sends no token headers and the tile server stays effectively public.
 */
public class MapTileAccessTokenMinter {
    private static final Logger logger = Logger.getLogger(MapTileAccessTokenMinter.class.getName());
     ...
     */
    private static final Pattern TILE_AUTH_TOKEN_PATTERN = Pattern.compile("[A-Za-z0-9_-]+");

    /**
     * Default tile-token TTL / bucket length in seconds when no TTL is configured.
     */
    public static final long DEFAULT_TTL_SECONDS = 120L;

    /**
     * Tile-server auth secrets keyed by id, parsed from the secrets configuration. Empty when authentication is
     * disabled.
     */
     ...
     */
    private final MapTileAccessTokenDTO disabledToken;
     ...
     */
    private final AtomicReference<CachedMapTileToken> cachedToken = new AtomicReference<>();

    /**
     * Builds a minter from the raw configuration strings as read from the bundle/system properties by {@link Activator}.
     *
     * @param secretsConfig the comma-separated {@code kid:secret} list (e.g. {@code k1:AbC-123_xyz,k2:Def-456_uvw}), or
     * {@code null}/blank to disable authentication. The comma separates entries and the first colon separates a kid from
     * its secret, so both kids and secrets must match the URL-safe base64url alphabet {@link #TILE_AUTH_TOKEN_PATTERN}
     * (no commas, colons, spaces or quotes inside a secret; there is no escaping). Entries that are malformed or violate
     * that alphabet are skipped with a warning rather than aborting server start-up.
     * @param kidConfig the id of the secret to sign new tokens with (a key from {@code secretsConfig}), or
     * {@code null}/blank to disable authentication.
     * @param ttlConfig the token TTL / bucket length in seconds as a decimal string, or {@code null}/blank to use
     * {@link #DEFAULT_TTL_SECONDS}.
     */
    public MapTileAccessTokenMinter(final String secretsConfig, final String kidConfig, final String ttlConfig) {
        ...
        disabledToken = new MapTileAccessTokenDTO(/* md5 */ null, /* expiresEpochSecond */ 0L, /* kid */ null,
                (ttlSeconds * 1000L) / 2L);
    }
     ...
     * {@link #TILE_AUTH_TOKEN_PATTERN} alphabet on both kids and secrets. Malformed or alphabet-violating entries are
        ...
        if (secretsConfig != null) {
            ...
            if (!trimmed.isEmpty()) {
                for (final String rawEntry : trimmed.split(",")) {
                    ...
                    final boolean valid = TILE_AUTH_TOKEN_PATTERN.matcher(kid).matches()
                            && TILE_AUTH_TOKEN_PATTERN.matcher(secret).matches();
                    if (valid) {
                        ...
                        logger.log(Level.WARNING, "Skipping malformed tile-server auth secret entry for kid \"{0}\": "
    ...
    private MapTileAccessTokenDTO getOrMintBucketToken(final String kid, final String secret) {
        ...
        final MapTileAccessTokenDTO result;
        final CachedMapTileToken cached = cachedToken.get();
    ...
    private synchronized MapTileAccessTokenDTO mintAndCacheBucketToken(final String kid, final String secret,
        ...
        final MapTileAccessTokenDTO result;
        final CachedMapTileToken existing = cachedToken.get();
        if (existing != null && existing.getBucketEndEpochSecond() == bucketEndEpochSecond
            ...
            final MapTileAccessTokenDTO token = new MapTileAccessTokenDTO(md5, expiresEpochSecond, kid,
                    refreshAfterMillis);
            cachedToken.set(new CachedMapTileToken(token, bucketEndEpochSecond, kid));
            result = token;
        }
        return result;
    }
